...
function renderCart(){
  ...
  if(!state.cart.length){ el.innerHTML='<div class="empty"><span class="material-symbols-outlined">shopping_cart</span><br>Your cart is empty.<br><br><button class="btn btn-primary" onclick="go(\'studio\')">Design something →</button></div>'; return; }
  let sub=0;
  let rows=state.cart.map((c,i)=>{ sub+=c.total; return `
    <tr>
      <td><img src="${c.thumb}" style="width:64px;border-radius:8px;border:1px solid var(--outline-variant)"></td>
      <td><b>${c.product}</b><br><span style="font-size:12px;color:var(--on-surface-variant)">${c.qty} pcs × ₹${c.unit}</span></td>
      <td><b>₹${c.total.toLocaleString('en-IN')}</b></td>
      <td><button class="btn btn-quiet btn-sm" onclick="rmCart(${i})">Remove</button></td>
    </tr>`; }).join('');
  ...
  el.innerHTML=`<table class="table"><tr><th>Preview</th><th>Item</th><th>Amount</th><th></th></tr>${rows}</table>
    <div class="panel" style="max-width:360px;margin-left:auto;margin-top:16px">
      ...
      <div class="price-line total"><span>Total</span><span>₹${tot.toLocaleString('en-IN')}</span></div>
      <button class="btn btn-primary btn-block" style="margin-top:12px" onclick="checkout(${tot})">Place order (Pay ₹${tot.toLocaleString('en-IN')})</button>
      <p style="font-size:11px;color:var(--on-surface-variant);margin-top:8px">Production: UPI / card via Razorpay. Digital proof in 2 hrs before printing.</p>
    </div>`;
}
...
async function renderOrders(){
  ...
  if(!state.user){ el.innerHTML='<div class="empty"><span class="material-symbols-outlined">inventory_2</span><br>Sign in to see your orders.<br><br><button class="btn btn-primary" onclick="openLogin()">Sign in →</button></div>'; return; }
  ...
  if(!orders.length){ el.innerHTML='<div class="empty"><span class="material-symbols-outlined">inventory_2</span><br>No orders yet.</div>'; return; }
...
async function renderAdmin(){
  ...
  el.innerHTML='<table class="table"><tr><th>Order</th><th>Customer</th><th>Total</th><th>Stage</th><th>Action</th></tr>'+
    orders.map(o=>`<tr><td><b>${o.id}</b></td><td>${o.customer}</td>
      ...
      <td>${o.status<STAGES.length-1?`<button class="btn btn-primary btn-sm" onclick="advance('${o.id}')">→ ${STAGES[o.status+1]}</button>`:'✅ Done'}</td></tr>`).join('')+'</table>';
}
